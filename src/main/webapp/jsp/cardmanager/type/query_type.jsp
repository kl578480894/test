...
<html>
	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		<label class="control-label label-default col-xs-12 " style="margin-bottom: 30px;">
			<h4>当前位置：会员卡类型设置 － 查询会员卡类型</h4>
		</label>
		<div class="container">
			<form method="post" action="<%=ctx %>/type/query.action" name="searchForm">
				<%-- 此处放条件查询 --%>
			</form>
		</div>
		<form action="<%=ctx %>/type/delete.action" method="post" name="delForm">
		<div class="container ">
			<table class="table table-hover table-striped table-condensed text-center" >
				...
				<tbody>
					<c:forEach items="${list }" var="type" varStatus="i">
						<tr>
							<td>${i.count}</td>
							<td><input type="checkbox" name="ids" value="${type.cardId}"></td>
							<td>${type.cardName}</td>
							<td>${type.cardNo}</td>
							<td>${type.cardJiage}</td>
							<td>${type.cardYucunjine}</td>
							<td>${type.cardInfo}</td>
							<td>
								<button onclick="editpage('${type.cardId}')" type="button" class="btn-sm btn-primary">修改</button>
							</td>
						</tr>
					</c:forEach>
				</tbody>
...
<script type="text/javascript">
	function addpage(){
		window.location.href = "<%=ctx%>/type/page.action";
	}
	
	function editpage(id){
		window.location.href = "<%=ctx%>/type/page.action?id="+id;
	}
