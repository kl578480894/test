...
<html>
	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		<label class="control-label label-default col-xs-12 " style="margin-bottom: 30px;">
			<h4>当前位置：紧急程度 － 查询紧急程度</h4>
		</label>
		<div class="container">
			<form method="post" action="<%=ctx %>/jjcd/queryCondition.action" name="searchForm">
				<%-- 此处放条件查询 --%>
			</form>
		</div>
		<form action="<%=ctx %>/jinjichengdu/delete.action" method="post" name="delForm">
		<div class="container ">
			<table class="table table-hover table-striped table-condensed text-center" >
				...
				<tbody>
				<c:forEach items="${list }" var="jjcd" varStatus="i">
					<tr>
						<td>${i.count}</td>
						<td><input type="checkbox" name="ids" value="${jjcd.cjcId}"></td>
						<td>${jjcd.cjcName}</td>
						<td>
							<button onclick="editpage('${jjcd.cjcId}')" type="button" class="btn-sm btn-primary">修改</button>
						</td>
					</tr>
					</c:forEach>
				</tbody>
...
<script type="text/javascript">
	function addpage(){
		window.location.href = "<%=ctx%>/jinjichengdu/addpage.action";
	}
	
	function editpage(id){
		window.location.href = "<%=ctx%>/jinjichengdu/editpage.action?id="+id;
	}
