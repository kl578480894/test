...
<html>
	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		<label class="control-label label-default col-xs-12 " style="margin-bottom: 30px;">
			<h4>当前位置：投诉方式 － 查询投诉方式</h4>
		</label>
		<div class="container">
			<form method="post" action="<%=ctx %>/tsfs/query.action" name="searchForm">
				<%-- 此处放条件查询 --%>
			</form>
		</div>
		<form action="<%=ctx %>/tsfs/delete.action" method="post" name="delForm">
		<div class="container ">
			<table class="table table-hover table-striped table-condensed text-center" >
				...
				<tbody>
				<c:forEach items="${list }" var="tsfs" varStatus="i">
					<tr>
						<td>${i.count}</td>
						<td><input type="checkbox" name="ids" value="${tsfs.ctfId}"></td>
						<td>${tsfs.ctfName}</td>
						<td>
							<button onclick="editpage('${tsfs.ctfId}')" type="button" class="btn-sm btn-primary">修改</button>
						</td>
					</tr>
					</c:forEach>
				</tbody>
...
<script type="text/javascript">
	function addpage(){
		window.location.href = "<%=ctx%>/tsfs/addpage.action";
	}
	
	function editpage(id){
		window.location.href = "<%=ctx%>/tsfs/editpage.action?id="+id;
	}
